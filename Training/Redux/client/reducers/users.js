export default function users(defStore=[],action){
    ...
    switch(action.type){
        ...
        let newData=[...defStore.slice(0,action.index),//This data is before theindex on which we are palying. Hence we need to past it without any change
            {...defStore[action.index],//Here we are getting the object on the tupple which we are playing with. This is also printing the tupple we are working with
                ...
                login:defStore[action.index].login+" Punit"},//This is working data on which we want to increment the things. This is actual working.This change is overriding the value initialized from the previous state at lin no 13
